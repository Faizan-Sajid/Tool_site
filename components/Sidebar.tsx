...
const Sidebar = () => {
  const { isSidebarOpen, closeSidebar, activeFilter, setActiveFilter, searchQuery, setSearchQuery } = useSidebar();
    ...
    >
      ...
      <div className="mb-6 px-1">
        <div className="relative group">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#3e3c38] group-focus-within:text-[#c9a84c] transition-colors" />
          <input
            type="text"
            placeholder="Search tools..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full bg-[#1a1e2e] border border-[rgba(255,255,255,0.07)] rounded-[10px] pl-9 pr-8 py-2 text-[12.5px] text-[#e6e3db] placeholder:text-[#3e3c38] focus:outline-none focus:border-[rgba(201,168,76,0.3)] transition-all"
          />
          {searchQuery && (
            <button
              onClick={() => setSearchQuery("")}
              className="absolute right-2.5 top-1/2 -translate-y-1/2 p-0.5 rounded-md hover:bg-[#252a41] text-[#3e3c38] hover:text-[#e6e3db] transition-colors"
            >
              <X className="w-3 h-3" />
            </button>
          )}
      ...
      {searchQuery.trim() ? (
        <div className="mb-6">
          <h3 className="text-[9.5px] uppercase tracking-[1.4px] text-[#3e3c38] font-bold mb-3 pl-[10px]">
            Search Results
          </h3>
          {filteredSearchTools.length > 0 ? (
            ...
            <p className="text-[11px] text-[#3e3c38] font-medium pl-3 mt-2 italic">
              No tools found...
            </p>
          )}
        </div>
        ...
        <>
          {/* Categories Section */}
          <div className="mb-2">
            <h3 className="text-[9.5px] uppercase tracking-[1.4px] text-[#3e3c38] font-bold mb-2 pl-[10px] mt-0">
              Categories
            </h3>
            <ul className="space-y-1">
              {categories.map((item) => (
                <li key={item.id}>
                  ...
                  >
                    ...
                    <span>{item.name}</span>
                    <span className="ml-auto bg-[#1a1e2e] text-[9.5px] font-bold text-[#3e3c38] p-[2px_7px] rounded-full min-w-[22px] text-center">
                      {item.count}
                    </span>
                  </button>
                </li>
          ...
          <div className="mt-[22px]">
            <h3 className="text-[9.5px] uppercase tracking-[1.4px] text-[#3e3c38] font-bold mb-2 pl-[10px]">
              Popular Tools
            </h3>
            <ul className="space-y-1">
